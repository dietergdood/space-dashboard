...
const NEWS_PROMPTS = {
  rklb: `RKLB Rocket Lab news last 48h. Fill with real recent events. JSON only: {"articles":[{"title":"Max 80 chars","sentiment":"pos|neg|neu","body":"news body in German","source":"Source name"}]}`,
  asts: `ASTS AST SpaceMobile news last 48h. Fill with real recent events. JSON only: {"articles":[{"title":"Max 80 chars","sentiment":"pos|neg|neu","body":"news body in German","source":"Source name"}]}`
};
...
const GOV_RKLB_PROMPT = `What US government contracts does Rocket Lab have in 2026? JSON only, no prose outside JSON: {"golden_dome":{"budget":"$X Mrd","rklb_anteil":"$X Mio","status":"active","desc":"RKLB role in program"},"sda":{"budget":"$X Mrd","rklb_auftraege":"$X Mio","status":"active","desc":"Tranche status"},"nasa":{"program":"VCLS","wert":"$X Mio","status":"active","desc":"launch contracts"},"space_force":{"program":"NSSL","wert":"$X Mio","status":"active","desc":"launch contracts"},"ausblick":"revenue outlook"}`;

const GOV_ASTS_PROMPT = `AST SpaceMobile (ASTS) regulatory status 2026: FCC, ITU, DoD, NTIA. Fill with real data. JSON only: {"fcc":{"status":"License status","frequenz":"Frequency band","desc":"FCC status details"},"itu":{"status":"Status","desc":"ITU coordination"},"dod":{"potenzial":"High/Medium/Low","desc":"DoD potential details"},"ntia":{"status":"Status","desc":"details"},"ausblick":"regulatory outlook"}`;

const SPCX_NEWS_PROMPT = `SpaceX news last 48 hours. Fill with real recent events. JSON only: {"articles":[{"title":"Max 80 chars","sentiment":"pos|neg|neu","body":"news body in German","source":"Source name"}]}`;
...
const SPCX_GOV_PROMPT = `SpaceX government contracts 2026: FAA Starship license, NASA Artemis HLS, Space Force NSSL, SEC IPO filing. Fill with real current data. JSON only: {"faa":{"status":"status","desc":"details"},"nasa":{"wert":"$X Mrd","desc":"details"},"space_force":{"wert":"$X Mrd","desc":"details"},"sec":{"status":"status","desc":"details"}}`;

const SPCX_CTX_PROMPT = `SpaceX company facts 2026. Fill with real data. JSON only: {"desc":"current status","tags":[{"text":"Rockets","type":"blue"},{"text":"Starlink","type":"green"},{"text":"IPO 2026","type":"amber"}],"stats":[{"label":"Valuation","val":"$XXX Mrd"},{"label":"Employees","val":"~13000"},{"label":"Launches 2025","val":"X"}]}`;
