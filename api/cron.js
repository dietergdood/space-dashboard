...
const NEWS_PROMPTS = {
  rklb: `RKLB Rocket Lab news last 48h. Fill with real recent events. JSON only: {"articles":[{"title":"Max 80 chars","sentiment":"pos|neg|neu","body":"2-3 sentences in German, no line breaks","source":"Source name"}]}`,
  asts: `ASTS AST SpaceMobile news last 48h. Fill with real recent events. JSON only: {"articles":[{"title":"Max 80 chars","sentiment":"pos|neg|neu","body":"2-3 sentences in German, no line breaks","source":"Source name"}]}`
};
...
const INSIDER_RKLB_PROMPT = `RKLB Rocket Lab insider trades last 90 days from SEC Form 4. Fill with real trades. JSON only, no slashes in name fields: {"trades":[{"name":"First Last","title":"CFO","type":"sell","shares":10000,"price":95.50,"value":955000,"date":"2026-05-15","note":"RSU vesting sale"}],"summary":"1 sentence in German","signal":"bearish"}`;

const INSIDER_ASTS_PROMPT = `ASTS AST SpaceMobile insider trades last 90 days from SEC Form 4. Fill with real trades. JSON only, no slashes in name fields: {"trades":[{"name":"First Last","title":"CEO","type":"sell","shares":5000,"price":120.00,"value":600000,"date":"2026-05-15","note":"RSU vesting sale"}],"summary":"1 sentence in German","signal":"neutral"}`;
...
const GOV_RKLB_PROMPT = `Rocket Lab (RKLB) US government contracts 2026: Golden Dome, SDA, NASA, Space Force. Fill with real data. JSON only: {"golden_dome":{"budget":"$X Mrd","rklb_anteil":"$X Mio","status":"Status","desc":"1-2 sentences RKLB relevance"},"sda":{"budget":"$X Mrd","rklb_auftraege":"X contracts $X Mio","status":"Status","desc":"1-2 sentences SDA Tranche"},"nasa":{"program":"VCLS/CLPS","wert":"$X Mio","status":"Status","desc":"1 sentence"},"space_force":{"program":"NSSL","wert":"$X Mio","status":"Status","desc":"1 sentence"},"ausblick":"1 sentence RKLB government revenue outlook"}`;

const GOV_ASTS_PROMPT = `AST SpaceMobile (ASTS) regulatory status 2026: FCC, ITU, DoD, NTIA. Fill with real data. JSON only: {"fcc":{"status":"License status","frequenz":"Frequency band","desc":"1-2 sentences FCC approval status"},"itu":{"status":"Status","desc":"1 sentence international coordination"},"dod":{"potenzial":"High/Medium/Low","desc":"1-2 sentences DoD potential"},"ntia":{"status":"Status","desc":"1 sentence"},"ausblick":"1 sentence ASTS regulatory outlook"}`;

const SPCX_NEWS_PROMPT = `SpaceX news last 48 hours. Fill with real recent events. JSON only: {"articles":[{"title":"Max 80 chars","sentiment":"pos|neg|neu","body":"2-3 sentences in German no line breaks","source":"Source name"}]}`;

const SPCX_SECTOR_PROMPT = `SpaceX market position 2026: Starlink users, Falcon 9 launches, Starship status, market share. Fill with real data. JSON only: {"intro":"1 sentence","ipo_status":"status","ipo_date":"date","bewertung":"$X Trd","cards":[{"emoji":"🛸","name":"Starlink","val":"X Mio users","val_color":"green","desc":"status"},{"emoji":"🚀","name":"Falcon 9","val":"X launches","val_color":"blue","desc":"market share"},{"emoji":"⭐","name":"Starship","val":"status","val_color":"amber","desc":"development"},{"emoji":"🏆","name":"Market share","val":"X%","val_color":"green","desc":"commercial"}]}`;
...
const SPCX_CTX_PROMPT = `SpaceX company facts 2026. Fill with real data. JSON only: {"desc":"1-2 sentences current status without special chars","tags":[{"text":"Rockets","type":"blue"},{"text":"Starlink","type":"green"},{"text":"IPO 2026","type":"amber"}],"stats":[{"label":"Valuation","val":"$XXX Mrd"},{"label":"Employees","val":"~13000"},{"label":"Launches 2025","val":"X"}]}`;
